...
if(event.type == KeyRelease){
  ...
  if(i == 36) // Too many key releases, ignore and clean keyboard
    ...
    if(_Wkeyboard[event.xkey.keycode] == 0)
      _Wkeyboard.key[event.xkey.keycode] = -1; // Press/release in the same frame
  }
}
@
\fimcodigo
...
// The position zero in our key array always means "any key":
W_ANY = 0;
...
if(event.type == SDL_KEYUP){
  ...
  if(i == 36) // Too many released keys, ignoring and cleaning keyboard:
    ...
    if(_Wkeyboard[event.key.keysym.scancode] == 0)
      _Wkeyboard.key[event.key.keysym.scancode] = -1; // Press/release: same frame
  }
}
@
\fimcodigo
...
if(event.type == WM_KEYUP){
  ...
  if(i == 36) // Too many released keys, ignoring and cleaning keyboard:
    ...
    if(_Wkeyboard[scancode] == 0)
      _Wkeyboard.key[scancode] = -1; // Simultaneous press/release
  }
}
@
\fimcodigo
...
but also its speed and acceleration, we should store the previous
speed. We use these variables for this:
...
to compute its speed, as we do not have a previous position. It makes
no sense compute some value for the mouse speed at this
moment. Likewise, in the frame after the initialization, despite
knowing a previous position and knowing how to compute the speed, we
do not have a previous speed, so we cannot compute the
acceleration. Only in the second frame after the initialization we can
compute both position, speed and acceleration. Until then, we should
let thes unknown values as zero. Because of this, we will use a
...
compute speed. And zero means that the initialization is over and we
know all values. We store this in the following variable:
...
buttons empty and also we initialize all buttons, the position, speed
and acceleration as zero:
