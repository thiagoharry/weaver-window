...
\subsubsecao{2.8.2. Changing Between Fullscreen and Windowed Mode on Windows}
...
void _Wtoggle_fullscreen(void){
  int size_x, size_y;
  DWORD window_style = GetWindowLongA(window, GWL_STYLE);
  if(window_style & WS_POPUP){ // We are in fullscreen. Entering windowed mode.
    ...
    if(last_window_size_x > 0 && last_window_size_y > 0)
      SetWindowPos(window, HWND_TOP, 0, 0, last_window_size_x,
                   last_window_size_y);
  } else{ // We are in windowed mode. Entering fullscreen.
    _Wget_screen_resolution(&size_x, &size_y);
