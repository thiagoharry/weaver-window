...
the function \monoespaco{wglGetProcAddress} as in the loading function
...
For example, to use the function \monoespaco{glCreateShader}, first we
declare it here:
...
Next we define the pointer here:
...
And then we load the function using our recently defined function:
...
Besides the shader creation function, we need the function that links
a source code to the created shader and another one to compile the
shader. This is done by the functions whose pointer are declared below:
...
The declared pointers are positioned here:
...
And we load the correct function to our pointers:
