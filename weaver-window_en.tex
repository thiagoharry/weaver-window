...
1. Introduction \indice{02}

1.1. Literary Programming and Notation \indice{03}

1.2. API Functions to Be Defined \indice{04}

2. Creating and Managing the Window \indice{05}

2.1. Getting Screen Resolution \indice{05}

2.1.1. Getting Screen Resolution on X \indice{05}

2.1.2. Getting Screen Resolution on Web Assembly \indice{07}

2.1.3. Getting Screen Resolution on Windows \indice{07}

2.2. Creating a Window \indice{07}

2.2.1. Creating a Window on X \indice{07}

2.2.2. Creating a Window on Web Assembly \indice{11}

2.2.3. Creating a Window on (Microsoft) Windows \indice{13}

2.2.4. Defining the  Window Creation Function for All Environments \indice{16}

2.3. Configuring OpenGL \indice{16}

2.3.1. Configuring OpenGL ES in X11 \indice{16}

2.3.2.Configuring OpenGL on Web Assembly \indice{18}

2.3.3. Configuring OpenGL on Windows \indice{19}

2.3.4. Choosing OpenGL Version \indice{43}

2.4. Closing a Window \indice{44}

2.4.1. Closing a Window on X \indice{44}

2.4.2. Closing a Window in Web Assembly \indice{44}

2.4.3. Closing a Window on Windows \indice{45}

2.5. Rendering the Window \indice{45}

2.5.1. Rendering Window on X \indice{45}

2.5.2. Rendering Window on Web Assembly \indice{45}

2.5.3. Rendering Window on Windows \indice{46}

2.6. Getting Window Size \indice{46}

2.6.1. Getting Window Size on X \indice{46}

2.6.2. Getting Window Size on Web Assembly \indice{46}

2.6.3. Getting Window Size on Windows \indice{47}

2.7. Checking Fullscreen Mode \indice{48}

2.7.1. Checking Fullscreen Mode on X \indice{48}

2.7.2. Checking Fullscreen Mode on Web Assembly \indice{48}

2.7.3. Checking Fullscreen Mode on Windows \indice{48}

2.8. Changing Between Fullscreen and Windowed Mode \indice{49}

2.8.1. Changing Between Fullscreen and Windowed Mode on X \indice{49}

2.8.2. Changing Between Fullscreen and Windowed Mode on Web Assembly \indice{51}

2.8.3. Changing Between Fullscreen and Windowed Mode on Windows \indice{52}

2.9. Resizing the Window \indice{53}

2.9.1. Resizing the Window on X \indice{53}

2.9.2. Resizing the Window on Web Assembly \indice{53}

2.9.3. Resizing the Window on Windows \indice{54}

3. Managing Input \indice{54}

3.1. Defining the Keyboard and Mouse \indice{54}

3.2. Reading the Keyboard \indice{56}

3.2.1. Reading the Keyboard on X \indice{57}

3.2.2. Reading the Keyboard on Web Assembly \indice{60}

3.2.3. Reading the Keyboard on Windows \indice{61}

3.2.4. Additional Code for Keyboard Support \indice{63}

3.3. Reading the Mouse \indice{65}

3.3.1. Reading the Mouse on X \indice{66}

3.3.2. Reading the Mouse on Web Assembly \indice{67}

3.3.3. Reading the Mouse on Windows \indice{68}

3.3.4. Additional Code for Mouse Support \indice{70}

3.4. Additional Input Functions \indice{72}

4. Estrutura Final do Arquivo \indice{73}
...
\subsecao{1.2. API Functions to Be Defined}
...
things: the SDL library, presented by Emscripten as the graphical API
and Javascript code to control the canvas where the SDL code is
rendered.

Now we include the Emscripten and SDL headers, with additional headers
needed by some macros that we will use:
...
canvas size where we will draw and initialize it with SDL code. The
HTML canvas should have the window inner size, except if
macro \monoespaco{W\_WINDOW\_NO\_FULLSCREEN} is defined and both
\monoespaco{W\_WINDOW\_RESOLUTION\_X}
and \monoespaco{W\_WINDOW\_RESOLUTION\_Y} have positive values. We
also ensure that the canvas is visible using Javascript, as it could
be hidden (we do it when we execute the function to close the window).
...
\subsubsecao{2.2.4. Defining the  Window Creation Function for All Environments}
...
Now we can create the OpenGL context. We choose the major and minor
OpenGL version using
macros \monoespaco{W\_WINDOW\_OPENGL\_MAJOR\_VERSION} and
\monoespaco{W\_WINDOW\_OPENGL\_MINOR\_VERSION}. With this information,
...
not personalized them to some value.
...
\subsubsecao{2.4.1. Closing a Window on X}
...
\subsubsecao{2.5.1. Rendering Window on X}
...
\subsubsecao{2.5.2. Rendering Window on Web Assembly}
...
\subsubsecao{2.5.3. Rendering Window on Windows}
...
\subsubsecao{2.7.1. Checking Fullscreen Mode on X}
...
\subsubsecao{2.8.1. Changing Between Fullscreen and Windowed Mode on X}
...
In the above function, we sent to the X server a request to change our
window size. However, this does not mean that when the
function \monoespaco{XResizeWindow} or \monoespaco{XResizeWindow}
return, the window is already in the new size. It is important to wait
until the size is successfully changed, otherwise we could return
incorrect or inconsistent values in other functions. For example, if
the window asks what is our window size after toggling the
fullscreen. Because of this in the abocve code we wait until the size
change is completed. This is more important when we exit

However, we cannot wait forever. We could be running in a window
manager that ignores our request to change the window size when we are
not in fullscreen mode. Because of this, we should have a maximum
waiting time. The code below shows how we implement the waiting
code. It waits until our window have the new size, but if this does
not happen in 0.1 seconds, it halts the waiting:

\iniciocodigo
@<X11: Wait for Resizing@>=
{ // Wait at most 0.1 for the resizing
...
And the function \monoespaco{gettimeofday} requires the following
header:
...
\subsubsecao{2.9.1. Resizing the Window on X}
...
\subsubsecao{2.9.2. Resizing the Window on Web Assembly}
...
\subsubsecao{2.9.3. Resizing the Window on Windows}

On Windows, resizing the window means
calling \monoespaco{SetWindowPos}. We do it only if we are not on
...
In the key array above, we will have one position for each keyboard
key where we will store a number to act as a timer for each key. The
...
the content of \monoespaco{keyboard.key[W\_ENTER]} without needing
...
The use of function \monoespaco{XkbKeycodeToKeysym} requires the
header:
...
\iniciocodigo
@<Update Mouse Position 'x' and 'y'@>=
@<Correct Mouse Position 'x' and 'y' if Forcing Landscape Mode@>
{
...
We also will correct above the mouse coordinate if the
macro \monoespaco{W\_FORCE\_LANDSCAPE} is defined and if the window
height is greater than the window width. In this case, we will rotate
the axis $x$ and $y$ of our coordinate system. We do the correction
using this code:
...
\subsecao{4. Final File Sctructure}
