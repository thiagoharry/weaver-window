...
Now we need to initializa OpenGL. First we need the appropriate
headers:
...
Now we need to configure OpenGL to draw in the screen, using double
buffering, with real color mode (not using paletes) and with
appropriate space to store red, blue, green, alpha and depth channels:
...
GLXFBConfig *framebuffer_config;
{
  ...
  int doubleBufferAttributes[] = {
    GLX_DRAWABLE_TYPE, GLX_WINDOW_BIT, // Draw to screen, not in 'pixmap'
    GLX_RENDER_TYPE, GLX_RGBA_BIT, // Use RGBA, not color paletes
    GLX_DOUBLEBUFFER, True, // Use double buffering: avoid 'flickering'
    GLX_RED_SIZE, 1, // At least 1 bit to red
    GLX_GREEN_SIZE, 1, // At least 1 bit to green
    GLX_BLUE_SIZE, 1, // At least 1 bit to blue
    GLX_ALPHA_SIZE, 1, // At least 1 bit to alpha channel
    GLX_DEPTH_SIZE, 1, // At least 1 bit to depth
    None
  };
