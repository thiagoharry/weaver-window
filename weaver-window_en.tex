...
status, we should call the following code that sets to empty our list
of pressed and released keys:
...
if(event.type == KeyPress){
  int i;
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].keycode == event.xkey.keycode) // Already being pressed
      break;
    if(pressed_keys[i].keycode == 0){ // The pressing is beginning now
...
extern int W_BACKSPACE, W_TAB, W_ENTER, W_UP, W_DOWN, W_LEFT, W_RIGHT, W_0, W_1,
           ...
           W_PAGE_UP, W_DELETE, W_END, W_PAGE_DOWN;
#endif
@
\fimcodigo
