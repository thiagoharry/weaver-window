...
2.6.3. Obtendo o Tamanho da Janela no Windows \indice{46}

2.7. Checando o modo de tela cheia \indice{47}

2.7.1. Checando modo de tela cheia no X11 \indice{47}

2.7.2. Checando modo de tela cheia no Web Assembly \indice{47}
...
2.8.3. Alternando entre Tela Cheia e Janela no Windows \indice{51}
...
2.9.3. Redimencionando a Janela no Windows \indice{53}
...
3.2.1. Lendo o Teclado no X \indice{57}
...
macro \monoespaco{W\_WINDOW\_NO\_FULLSCREEN} com valores positivos
para \monoespaco{W\_WINDOW\_RESOLUTION\_X}
...
\subsubsecao{2.4.1. Fechando uma Janela no X11}
...
\subsubsecao{2.4.3. Fechando uma Janela no Windows}
...
\subsubsecao{2.5.1. Renderizando a Janela no X} 
...
\subsubsecao{2.5.2. Renderizando a Janela no Emscripten} 
...
\subsubsecao{2.5.3. Renderizando a Janela no Windows}
...
\subsubsecao{2.6.2. Obtendo o Tamanho da Janela no Web Assembly}
...
\subsecao{2.7. Checando o modo de tela cheia}

Aqui iremos definir as funções que verificam se estamos em modo de
tela cheia ou não.

\subsubsecao{2.7.1. Checando modo de tela cheia no X11}
...
função \monoespaco{XGetWindowAttributes} se a nossa janela tem tal
...
\subsubsecao{2.7.2. Checando modo de tela cheia no Web Assembly}
...
\subsubsecao{2.7.3. Checando modo de tela cheia no Windows}

Detectar a tela cheia no Windows envolve usar a
função \monoespaco{SHQueryUserNotificationState} para obter o estado
da tela. Se tivermos uma janela em tela-cheia, saberemos disso se
obtiveros como resultado \monoespaco{QUNS\_BUSY}:
...
\subsecao{2.8. Alternando entre Tela Cheia e Janela}
...
\subsubsecao{2.8.1. Alternando entre Tela Cheia e Janela no X11}
...
Mas não podemos esperar para sempre. Pois pode ser que o gerenciador
...
\subsubsecao{2.8.2. Alternando entre Tela Cheia e Janela no Web Assembly}
...
booleana, a \monoespaco{fullscreen\_mode} que nos diz se estamos em
tela-cheia (verdadeira ou simulada) ou não. Em contraste, a
função \monoespaco{\_Wis\_fullscreen} apenas nos informa se estamos em
...
\subsubsecao{2.8.3. Alternando entre Tela Cheia e Janela no Windows}
...
por meio de \monoespaco{GetWindowLongPtrA} e \monoespaco{SetWindowLongPtrA}:
...
\subsecao{2.9. Redimencionando a Janela}
...
\subsubsecao{2.9.1. Redimencionando a Janela no X11}

Redimencionar o X11 significa usar a
função \monoespaco{XResizeWindow}, que já viemos usando em outras
...
usando \monoespaco{XSetWMNormalHints}. Fazemos isso para impedir que o
...
\subsubsecao{2.9.2. Redimencionando a Janela no Web Assembly}
...
\subsubsecao{2.9.3. Redimencionando a Janela no Windows}

No Windows mover a janela significa usar a
função \monoespaco{SetWindowPos}, o que fazemos somente se não
...
\subsubsecao{3.2.1. Lendo o Teclado no X}
...
O uso da função \monoespaco{XkbKeycodeToKeysym} requer o cabeçalho:
...
\subsubsecao{3.2.2. Lendo o Teclado no Web Assembly}
...
\subsubsecao{3.2.3. Lendo o Teclado no Windows}
...
uma leitura anterior de sua posição e velocidade. Então não faz sentido
...
@<Atualiza Posição do Mouse 'x' e 'y'@>=
{
  @<Corrige Posição do Mouse 'x' e 'y' se Forçamos Modo Paisagem@>
...
Acima nós também podemos precisar corrigir a posição do mouse se a
macro \monoespaco{W\_FORCE\_LANDSCAPE} estiver definida. Neste caso,
se a altura de nossa janela for maior que a largura, vamos inverter
os eixos $x$ e $y$. O que significa que vamos trocar as posições $x$ e
$y$ lidas do mouse:
...
\secao{4. Estrutura Final do Arquivo}
