...
Vamos também criar uma variável estática que irá nos ajudar a
memorizar qual o tamanho atual da nossa janela (não da tela
inteira). Essa informação será útil futuramente, pois usá-la é
importante para tratar corretamente as coordenadas de nossa
janela. Esse valor será apenas usado internamente. Se o usuário
perguntar por mieo de alguma função da API qual o tamanho da janela,
ao invés de usar o valor memorizado, iremos verificar o valor atual na
hora e atualizar com base nisso o valor memorizado:

\iniciocodigo
@<Variáveis Locais@>=
static int window_size_x = 0, window_size_y = 0;
@
\fimcodigo
...
if(event.type == ButtonPress){
  int i;
  usigned button = event.xbutton.button;
...
if(event.type == MotionNotify){
  int x, y;
  x = event.xmotion.x;
  y = (window_size_x - 1) - event.xmotion.y;
...
#if !defined(_WIN32) && !defined(__EMSCRIPTEN__)
{
  ...
  ret = XQueryPointer(display, window, &root_return, &child_return,
                      &root_x_return, &root_y_return, &(_Wmouse.x),
                      &(_Wmouse.y), &mask_return);
  // Transformando coordenada 'y':
  _Wmouse.y = (window_size_x - 1) - _Wmouse.y;
}
#endif
@
\fimcodigo
...
if(event.type == SDL_MOUSEBUTTONDOWN){
  usigned button = event.obutton.button;
  @<Adiciona 'button' à Lista de Botões Pressionados@>
}
@
\fimcodigo
...
if(event.type == SDL_MOUSEBUTTONUP){
  usigned button = event.button.button;
  @<Remove 'botton' da Lista de Botões Pressionados e Adiciona à Botões Soltos@>
}
@
\fimcodigo
...
if(event.type == SDL_MOUSEMOTION){
  int x, y;
  x = event.motion.x;
  y = (window_size_x - 1) - event.motion.y;
  @<Atualiza Posição do Mouse 'x' e 'y'@>
}
@
\fimcodigo
...
#if defined(__EMSCRIPTEN__)
{
  SDL_GetMouseState(&(_Wmouse.x), &(_Wmouse.y));
  // Transformando coordenada 'y':
  _Wmouse.y = (window_size_x - 1) - _Wmouse.y;
}
#endif
...
if(event.type == WM_MOUSEMOVE){
  ...
  y = (window_size_x - 1) - (event.lParam >> 16);
  @<Atualiza Posição do Mouse 'x' e 'y'@>
}
@
\fimcodigo
...
#if defined(_WIN32)
{
  ...
  // Armazena a coordenada no mouse, transformando a coordenada 'y':
  _Wmouse.x = point.x;
  _Wmouse.y = (window_size_x - 1) - point.y;
}
#endif
@
\fimcodigo
...
@<Remove 'botton' da Lista de Botões Pressionados e Adiciona à Botões Soltos@>=
{
  ...
  for(i = 0; i < 4; i ++){ // Removendo da lsta de botões pressionados
    if(pressed_buttons[i].button == button){
      int j;
      stored_time = pressed_buttons[j].time;
      for(j = i; j < 3; j ++){
...
E agora mostramos o código para atualizar a psição do mouse, bem como
sua velocidade e aceleração (a depender do quanto avançamos na
inicialização do mouse). N rodamos este código ao detectar o movimento
do mouse e ler aí um novo valor $x$ e $y$:
