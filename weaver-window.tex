...
unsigned released_keys[32]; // Que teclas foram soltas?
@
\fimcodigo

Como parte da inicialização de nosso teclado, e também para reiniciar
o estado dele, devemos chamar sempre o código abaixo que torna vazia
nossa lista de teclas pressionadas e soltas. Isso torna a lista vazia
pelo simples fato de que nesta lista iremos tratar a primeira
ocorrênciade uma tecla com valor zero como sendo o fim da lista, assim
como o caractere nulo marca o fim de uma string.
...
if(event.type == KeyRelease){
  ...
  if(i == 36) // Muitas teclas soltas: ignorando e limpando teclado:
    ...
    if(keyboard[event.xkey.keycode] == 0)
      keyboard.key[event.xkey.keycode] = -1; // Pressionou/soltou no mesmo frame
  }
}
@
\fimcodigo
...
if(event.type == SDL_KEYUP){
  ...
  if(i == 36) // Muitas teclas soltas: ignorando e limpando teclado:
    ...
    if(keyboard[event.key.keysym.scancode] == 0)
      keyboard.key[event.key.keysym.scancode] = -1; // Aperta/solta no mesmo frame
  }
}
@
\fimcodigo
...
if(event.type == WM_KEYUP){
  ...
  if(i == 36) // Too many released keys, ignoring and cleaning keyboard:
    ...
    if(keyboard[scancode] == 0)
      keyboard.key[scancode] = -1; // Pressionou/soltou simultaneamente
  }
}
@
\fimcodigo
