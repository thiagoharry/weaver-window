...
Agora vamos inicializar um contexto OpenGL. Para isso primeiro vamos
inserir os cabeçahos apropriados:
...
Agora vamos configurar o OpenGL para desenhar na tela usando duplos
buffers, usando modo de cores reais (ao invés de paletas de cores) e
com espaço apropriado para armazenar os canais vermelho, azul, verde,
alfa e profundidade:
...
GLXFBConfig *framebuffer_config;
{
  ...
  int doubleBufferAttributes[] = {
    GLX_DRAWABLE_TYPE, GLX_WINDOW_BIT, //Desenhe na tela, não em 'pixmap'
    GLX_RENDER_TYPE, GLX_RGBA_BIT, // Usar RGBA, não paletas de cores
    GLX_DOUBLEBUFFER, True, // Usar buffer duplo, evita 'flickering'
    GLX_RED_SIZE, 1, // Ao menos 1 bit de vermelho
    GLX_GREEN_SIZE, 1, // Ao menos 1 bit de verde
    GLX_BLUE_SIZE, 1, // Ao menos 1 bit de azul
    GLX_ALPHA_SIZE, 1, // Ao menos 1 bit de canal alfa
    GLX_DEPTH_SIZE, 1, // Ao menos 1 bit de profundidade
    None
  };
...
\subsecao{2.2. Criando uma ``Janela'' em Navegador Web}
...
\subsecao{2.3. Criando uma Janela no Windows}
...
\subsecao{2.X. Função de Criação de Janelas}
...
\subsecao{2.X+1. Fechando uma Janela no X}
...
\subsecao{2.X+2. Fechando uma Janela em Web Assembly}
...
\subsecao{2.X+3. Fechando uma Janela no Windows}
