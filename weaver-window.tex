...
struct __Wkeyboard{
  long key[W_KEYBOARD_SIZE]; // Vetor de teclas: contador de tempo para elas
} _Wkeyboard;
@
\fimcodigo
...
O mouse será representado pela seguinte variável global de estrutura:
...
de quase 256 teclas diferentes. E na nossa representação de teclado
colocamos um espaço adicional para representar se uma tecla qualquer
...
No momento em que detectamos uma tecla sendo pressionada, armazenamos
ela, a identificando pelo seu código de tecla. Fazemos isso somente se
a tecla já não estava sendo pressionada:
...
if(event.type == KeyPress){
  int i;
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].key == event.xkey.keycode) // Já tava pressionada
      break;
    if(pressed_keys[i].key == 0){ // Começou a ser pressionada agora
      ...
      // Atualizando teclado:
      _Wkeyboard.key[event.xkey.keycode] = 1;
      break;
    }
  }
...
Quando detectamos uma tecla sendo solta, a removemos da lista de
teclas sendo pressionadas e a adicionamos à lista de teclas soltas:
...
if(event.type == KeyRelease){
  int i;
  long stored_time = -1;
  for(i = 0; i < 36; i ++){ // Removendo da lista de tecas pressionadas
    if(pressed_keys[i].key == event.xkey.keycode){
      int j;
  ...
  for(i = 0; i < 36; i ++){ // Adicionando à lista de teclas soltas:
    if(released_keys[i] == 0)
      released_keys[i] = event.xkey.keycode;
  }
  if(i == 36) // Muitas teclas soltas: ignorando e limpando teclado:
    _Wkeyboard.key[event.xkey.keycode] = 0;
  else{
    // Atualizando teclado:
    _Wkeyboard.key[event.xkey.keycode] = - (long) (current_time - stored_time);
    if(_Wkeyboard[event.xkey.keycode] == 0)
      _Wkeyboard.key[event.xkey.keycode] = -1; // Pressionou/soltou no mesmo frame
  }
}
@
\fimcodigo

Embora o cóigo acima trate corretamente os códigos de teclas, um
...
de \monoespaco{keyboard[W\_ENTER]} sem ter que saber qual o código
...
suportamos no teclado para assim darmos a eles nomes adequados. O
código para inicializar o teclado fica sendo então:

\iniciocodigo
@<Inicialização de Teclado@>=
@<Limpando Lista de Teclas Pressionadas e Soltas@>
// A última posição de nosso vetor de teclas sempre representa "qualquer tecla":
W_ANY = W_KEYBOARD_SIZE - 1;
// Outras posições são definidas pelo ambiente. Mas 0 é "nenhuma tecla":
#if !defined(_WIN32) && !defined(__EMSCRIPTEN__)
{
  int i;
  for(i = 8; i < 256; i ++){
...
E elas são inicializadas como sendo o valor zero. Que por padrão
representa um local do vetor de teclas do teclado que sempre será nulo
e nunca será pressionada ou solta. Só depois da inicialização podemos
saber qual posição corresponde a qual tecla ajustando o valor de tais
variáveis:
...
int W_BACKSPACE, W_TAB, W_ENTER, W_UP, W_DOWN, W_LEFT, W_RIGHT, W_0, W_1,
    ...
    W_F3, W_F4, W_F5, W_F6, W_F7, W_F8, W_F9, W_F10, W_F11, W_F12,
    W_LEFT_SHIFT, W_RIGHT_SHIFT, W_LEFT_ALT, W_RIGHT_ALT, W_LEFT_CTRL,
    W_RIGHT_CTRL, W_SPACE, W_A, W_B, W_C, W_D, W_E, W_F, W_G, W_H, W_I, W_J, W_K,
    W_L, W_M, W_N, W_O, W_P, W_Q, W_R, W_S, W_T, W_U, W_V, W_W, X_X, W_Y, W_Z,
    W_INSERT, W_HOME, W_PAGE_UP, W_DELETE, W_END, W_PAGE_DOWN, W_ESC;
#endif
@
\fimcodigo
...
if(event.type == SDL_KEYDOWN){
  int i;
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].key == event.key.keysym.scancode) // Já pressionada
      break;
    if(pressed_keys[i].key == 0){ // Começou a ser pressionada agora
      pressed_keys[i].key = event.key.keysym.scancode;
      pressed_keys[i].time = current_time;
      // Atualizando teclado:
      _Wkeyboard.key[event.key.keysym.scancode] = 1;
      break;
    }
  }
  if(i == 36) continue; // Ignorando: muitas teclas pressionadas
}
@
\fimcodigo
...
if(event.type == SDL_KEYUP){
  int i;
  long stored_time = -1;
  for(i = 0; i < 36; i ++){ // Removendo da lista de teclas pressionadas
  ...
  if(i == 36) // Muitas teclas soltas: ignorando e limpando teclado:
    _Wkeyboard.key[event.key.keysym.scancode] = 0;
  else{
    // Atualizando teclado:
    _Wkeyboard.key[event.key.keysym.scancode] =
      -(long) (current_time - stored_time);
    if(_Wkeyboard[event.key.keysym.scancode] == 0)
      _Wkeyboard.key[event.key.keysym.scancode] = -1; // Aperta/solta: mesmo frame
  }
}
@
\fimcodigo
...
if(event.message == WM_KEYDOWN){
  ...
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].key == scancode) // Já pressionada
      break;
    if(pressed_keys[i].key == 0){ // Sendo pressionada agora
      pressed_keys[i].key = scancode;
      pressed_keys[i].time = current_time;
      // Atualizando teclado:
      _Wkeyboard.key[scancode] = 1;
      break;
    }
  }
  if(i == 36) continue; // Ignorando: muitas teclas pressionadas
...
Note que uma das diuferenças aqui é que para extrair o ``scancode'' da
...
if(event.type == WM_KEYUP){
  ...
  for(i = 0; i < 36; i ++){ // Adicionando à lista de tecas soltas
    if(released_keys[i] == 0)
      released_keys[i] = scancode;
  }
  if(i == 36) // Too many released keys, ignoring and cleaning keyboard:
    _Wkeyboard.key[scancode] = 0;
  else{
    // Updating Keyboard:
    _Wkeyboard.key[scancode] = -(long)(current_time - stored_time);
    if(_Wkeyboard[scancode] == 0)
      _Wkeyboard.key[scancode] = -1; // Pressionou/soltou simultaneamente
  }
}
@
\fimcodigo
...
\subsecao{3.2.4. Cigo Adicional para Suporte de Teclado}
...
atualiza em cada frame o número representando a quanto tempo cada
tecla está sendo pressionada. Para isso, precisamos iterar a lista de
teclas pressionadas e para cada uma delas, atualizar o número
armazenado na posição do vetor de teclas de nossa estrutura de teclado
que corresponde a tal tecla pressionada. Nós fazemos isso na mesma
função que checa por entrada e por novas teclas pressionadas. Mas
posicionamos tal código somente depois que a lista de teclas
pressionadas do frame foi atualizada:
...
@<Depois de Obter Eventos da Janela@>=
{
  ...
  _Wkeyboard[W_ANY] = (pressed_keys[0].key != 0);
  for(i = 0; i < 32; i ++){
...
ser solto. Para isso, antes de lermos a entrada da janela, devemos
iterar sobre a lista de teclas que foram pressionadas no último frame
e para cada uma delas ajustar o número de cada tecla para zero e
esvaziar a lista de teclas soltas:
...
bter aceleração. E o valor 0 indica que a inicialização terminou. A
