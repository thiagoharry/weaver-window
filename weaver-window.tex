...
ser invocada sempre após a janela já ter sido criada.
...
macros \monoespaco{W\_WINDOW\_DEFAULT\_RESOLUTION\_X}
e \monoespaco{W\_WINDOW\_DEFAULT\_RESOLUTION\_Y} sendo que se elas
...
E também assume termos o cabeçalho para imprimir mensagens na saída de
erro e para encerrar o programa se algo der errado:
...
sermos avisados deste evento). O código para isso é:
...
\subsecao{2.X. Estrutura da Função de Criação de Janelas}

Nossa função de geração de nova janela então executará o código
adequado, dependendo do ambiente gráfico em que estamos:
...
conexão com o servidor. Isso é feito chamando as duas funções abaixo:
