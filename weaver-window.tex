...
Por exemplo, para usar a função \monoespaco{glCreateShader}, primeiro
precisamos declarar um ponteiro para ela:
...
Em seguida carregamos a função com o código da função que recém
definimos acima:
...
O tipo de shader a ser criado é escolhido passando uma das macros abaixo:
...
Além da função para criar um shader, precisamos da função que liga um
código-fonte de um shader ao shader em si e outra que o compila. Isso
é feito pela funções cujo ponteiros são declarados abaixo:
...
Os ponteiros declarados são posicionados aqui:
...
E carregamos a função correta para os ponteiros:
