...
o estado dele, devemos chamar sempre o seguinte código que torna vazia
nossa lista de teclas pressionadas e soltas:
...
if(event.type == KeyPress){
  int i;
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].keycode == event.xkey.keycode) // Já tava pressionada
      break;
    if(pressed_keys[i].keycode == 0){ // Começou a ser pressionada agora
      pressed_keys[i].keycode = event.xkey.keycode;
...
if(event.type == KeyRelease){
  ...
  for(i = 0; i < 36; i ++){ // Removendo da lista de tecas pressionadas
    if(pressed_keys[i].keycode == event.xkey.keycode){
      int j;
      stored_time = pressed_keys[j].time;
      for(j = i; j < 35; j ++){
        pressed_keys[j].keycode = pressed_keys[j + 1].keycode;
        pressed_keys[j].time = pressed_keys[j + 1].time;
        if(pressed_keys[j].keycode == 0)
          break;
      }
      pressed_keys[35].keycode = 0;
      break;
    }
  }
...
int W_BACKSPACE, W_TAB, W_ENTER, W_UP, W_DOWN, W_LEFT, W_RIGHT, W_0, W_1,
    ...
    W_INSERT, W_HOME, W_PAGE_UP, W_DELETE, W_END, W_PAGE_DOWN;
#endif
@
\fimcodigo
...
Assim como no X a tecla física era chamada de \monoespaco{Keycode} e o
símbolo associado à ela era o \monoespaco{Keysym}, no SDL as
estruturas são chamadas respectivamente de \monoespaco{SDL\_Scancode}
e \monoespaco{SDL\_Keycode}. Para detectarmos se uma tecla foi
prssionada, usamos o seguinte código que lê
o \monoespaco{SDL\_Scancode} (tecla física):
...
if(event.type == SDL_KEYDOWN){
  int i;
  for(i = 0; i < 36; i ++){
    if(pressed_keys[i].keycode == event.key.keysym.scancode) // Já pressionada
      break;
    if(pressed_keys[i].keycode == 0){ // Começou a ser pressionada agora
      pressed_keys[i].keycode = event.key.keysym.scancode;
      pressed_keys[i].time = current_time;
      // Atualizando teclado:
...
if(event.type == SDL_KEYUP){
  ...
  for(i = 0; i < 36; i ++){ // Removendo da lista de teclas pressionadas
    if(pressed_keys[i].keycode == event.key.keysym.scancode){
      int j;
      stored_time = pressed_keys[j].time;
      for(j = i; j < 35; j ++){
        pressed_keys[j].keycode = pressed_keys[j + 1].keycode;
