...
Configurar o OpenGL no Windows é um bocado trabahoso. Pra começar, a
função que cria contexto OpenGL definida por padrão,
a \monoespaco{wglCreateContext} cria um contexto muito primitivo, sem
suporte à funções mais recentes OpenGL. Mas existe definida na prática
uma outra função que cria
contexto: \monoespaco{wglCreateContextAttribsARB}, a qual permite que
façamos coisas básicas como pedir por uma versão especípica do OpenGL
com suporte à funções mais modernas.
...
nossa janela e carregar como extensões as funções que precisamos. Mas
o problema é que já tínhamos carregado o contexto primitivo antes e
associado ele à janela atual. Por isso precisamos também destruir a
janela anterior e criar uma nova.

Para começar, vamos criar um contexto OpenGL básico que será
temporário. Primeiro precisamos criar a configuração do formato de
pixels dele. Usamos para isso a configuração recomendada na
documentação:
...
Todo esse trabalho criou e tornou atual um contexto OpenGL
básico. Tendo um contexto básico, agora podemos carregar a função que
criará o contexto OpenGL final.
...
Para usar esta função e carregar novas funções, primeiro precisamos
declarar um ponteiro que armazenará a posição de memória onde está a
nova função que foi carregada. A declaração do ponteiro requer que
saibamos exatamente o tipo da função. No caso da função que carregará
o contexto final e sua função auxiliar que escolhe a configuração do
formato de pixel, a declaração de seus ponteiros é:
...
Esta é só a declaração externa que irá para \monoespaco{window.h}. A
declaração real como variável global posicionaremos
em \monoespaco{window.c} assim:
...
Se conseguimos carregar as duas funções com sucesso, então devemos
destruir a janela e o contexto atual para criá-los de novo:
...
E agora criamos a janela novamente, buscando respeitar o tamanho
pedido da janela. Se estivernos em tela cheia, só usamos o máximo do
tamanho disponível, se não estivermos aí escolhemos o tamanho máximo
levando em conta a barra do menu iniciar, mas se o usuário definiu um
tamanho específico e não estamos em tela-cheia, isso é o que terá
prioridade:
...
Vamos agora configurar o formato de pixel do verdadeiro contexto OpenGL
que conseguiremos criar agora:
...
Temos também que definir algumas das macros que estamos usando:
